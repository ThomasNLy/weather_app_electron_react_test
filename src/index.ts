...
interface IWeatherDataCacheCollection {
    timeStamp: number;
    weatherDataCollection: object;
}
...
const iconPath = app.isPackaged
    ? path.join(process.resourcesPath, "assets", "app_icons", "icon.ico")
    : path.join(__dirname, "..", "..", "src", "assets", "app_icons", "icon.ico");

Menu.setApplicationMenu(null); // hide default electron menu bar
...
app.on("window-all-closed", () => {
    savelocationData();
    cacheWeatherDataToLocalStorage();
...
/*
use fs system to save it to json file to be loaded in later
limit number of items and delete any if older than 1 hour
when reading and loading them in 
*/
async function cacheWeatherDataToLocalStorage() {
    const userData: string = app.getPath("userData");
    const filePath: string = path.join(userData, "weatherDataCache.json");
    let jsonFormat: IWeatherDataCacheCollection = {
        timeStamp: Date.now(),
        weatherDataCollection: Object.fromEntries(weatherDataCache),
    };
    let jsonWeatherDataCache: string = JSON.stringify(jsonFormat, null, 4);
    try {
        await fs.writeFile(filePath, jsonWeatherDataCache, "utf8");
    } catch (error) {
        console.log(error);
    }
}

async function loadCachedWeatherDataFromLocalStorage() {
    const userData: string = app.getPath("userData");
    const filePath: string = path.join(userData, "weatherDataCache.json");
    try {
        const data: string = await fs.readFile(filePath, "utf8");
        const weatherDataCacheCollection: IWeatherDataCacheCollection = JSON.parse(data);
        const weatherDataJsonObject = weatherDataCacheCollection.weatherDataCollection;
        weatherDataCache =
            Date.now() - weatherDataCacheCollection.timeStamp < 3600000
                ? new Map(Object.entries(weatherDataJsonObject))
                : new Map();
    } catch (error) {
        console.log("unable to load cached data as none exist", error);
    }
}
